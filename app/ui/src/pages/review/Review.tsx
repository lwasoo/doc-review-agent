...
function Review() {
  ...
  const metrics = useMemo(() => {
    ...
    const highOpen = issues.filter((i) => issueRiskLevel(i.type, i.risk_level) === '高' && normalizeIssueStatus(i.status as unknown as string) === IssueStatus.NotReviewed).length
  ...
  const runCheck = useCallback((force = false) => {
    if (!docId || checkInProgress) return
    setCheckInProgress(true)
    setCheckError(undefined)
    setCheckComplete(false)
    setIssues([])

    const apiPath = buildIssuesPath(docId, {
      force,
      ruleIds: enabledRuleIdsRef.current,
      reviewParty,
    })
    ...
    streamApi(
      apiPath,
      (msg) => {
        // Ignore SSE keep-alive/heartbeat frames.
        if (!msg.event || msg.event === 'message' || msg.data === 'keep-alive') {
          return
        }
        switch (msg.event) {
          ...
          case APIEvent.Complete: {
            ...
            setLastCompletedReviewParty(reviewParty)
            break
          }
          default:
  ...
  useEffect(() => {
    ...
    setLastCompletedReviewParty(undefined)
    setCheckInProgress(false)
    setCheckComplete(false)
    setCheckError(undefined)
    setIssues([])
      ...
      <Card className={`${classes.panel} ${classes.left}`}>
        ...
        <div className={classes.leftList}>
          ...
          {!checkInProgress && filteredIssues.length === 0 && (
            <div className={classes.noIssues}>{hasStartedReview ? '暂无问题' : '请选择立场后点击“开始审阅”'}</div>
          )}
        </div>
      </Card>
        ...
        <div className={classes.toolbar}>
          ...
          <div className={classes.toolbarSection}>
            ...
            <Dropdown
              ...
              selectedOptions={[reviewParty]}
              onOptionSelect={(_, data) => setReviewParty((data.optionValue as ReviewParty) ?? 'both')}
              disabled={checkInProgress}
            >
              <Option value="both">双方平衡</Option>
              <Option value="party_a">保护甲方</Option>
              <Option value="party_b">保护乙方</Option>
            </Dropdown>
            <Button
              size="small"
              appearance="primary"
