...
export type ReviewParty = 'party_a' | 'party_b' | 'both'
...
export function buildIssuesPath(docId: string, options?: { force?: boolean; ruleIds?: string[]; reviewParty?: ReviewParty }) {
  const params = new URLSearchParams()
  if (options?.force) params.set('force', 'true')
  if (options?.ruleIds?.length) {
    options.ruleIds.forEach((id) => params.append('rule_ids', id))
  }
  if (options?.reviewParty) {
    params.set('review_party', options.reviewParty)
  }
  const queryString = params.toString()
  return `${docId}/issues${queryString ? `?${queryString}` : ''}`
}
export async function streamApi(
  ...
  fetchEventSource(apiBaseUrl + path, {
    ...
    onclose() {
      // Prevent silent reconnect loops that can repeatedly trigger expensive review requests.
      if (!abortControllerRef.signal.aborted) {
        throw new FatalError('审核流已关闭，请手动重试')
      }
    },
    onerror(err) {
