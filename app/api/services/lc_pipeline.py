...
def _build_system_prompt(custom_rules: List[ReviewRule] | None = None, review_party: str = "both") -> str:
    ...
    return f"""你是一位法律合同审阅专家。仅识别“真实且可行动”的法律风险问题。

审查立场：{party_text}
重点关注：对{party_text}不利、权责失衡、履约/付款/违约/争议解决风险、合规风险与条款可执行性。
...
排除项（不要报）：
- 各类序号/编号：1、(1)、一、①、a 等
- 表单占位符：____年____月、___元、空白下划线、待填写字段
- 勾选符号：口、□、☐、○
- 纯格式元素：分隔线、无语义标点
- 通用模板词：甲方、乙方、签字、盖章等单独出现
...
def _build_guidance(custom_rules: List[ReviewRule] | None = None, review_party: str = "both") -> str:
    ...
    lines = [
        "审核指南：",
        f"- 立场：{party_text}（优先判断对{party_text}是否不利）",
        "- Liability: 免责/赔偿/违约责任明显失衡",
        "- Payment: 付款触发、账期、验收、对账机制不合理",
        "- Termination: 单方解除权过宽、违约触发不清、补救期缺失",
        "- Obligation: 义务对象、时限、范围、标准、交付口径不明确",
        "- Compliance: 违法合规风险或合规责任边界不清",
        "- Dispute: 管辖/仲裁/举证/通知条款明显偏向一方",
        "- 仅输出高置信问题；低置信且影响小的内容可忽略",
    ]
