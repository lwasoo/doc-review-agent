...
from common.models import DismissalFeedbackModel, Issue, IssueStatusEnum, ModifiedFieldsModel
...
async def get_pdf_issues(
    ...
    review_party: Optional[str] = Query(None, description="Protect party: party_a, party_b, both"),
    ...
    Args:
        ...
        review_party (str): Optional review perspective (party_a, party_b, both)
    ...
    try:
        ...
        # Hard safety: while a review is running, ignore force=true to avoid costly duplicate reruns.
        if force and await issues_service.is_review_active(doc_id):
            logging.warning(f"Ignoring force=true for {doc_id}: review already running")
            force = False

        # If force=true, clear in-memory review cache and delete stored issues before re-review.
        if force:
            await issues_service.clear_review_cache(doc_id)
            if stored_issues:
        ...
        if stored_issues:
            ...
            issues_stream = issues_service.initiate_review(
                str(pdf_path), user, date_time, custom_rules, review_party, force=force
            )

            async def issues_events():
                try:
                    # Initial heartbeat to reduce idle disconnect/retry loops.
                    yield issues_event([])
